...
class Grid {
  ...
  Grid(int _sectorsDown, int _sectorsAcross) {
  ...
  void makeSectors() {
    int sectorW = width / sectorsAcross;
    int sectorH = height / sectorsDown;

    for (int y = 0; y < sectorsDown; y++) {
      for (int x = 0; x < sectorsAcross; x++) {
        PVector secCorner = new PVector(x*sectorW, y*sectorH);
